import { getMovieCredits, getMovieDetailData } from "@/libs/api/movie";
import { movieRuntime } from "@/libs/helper";
import Image from "next/image";

export default async function DetailInfo({ id }: { id: number }) {
  ...
  const casting = credits.cast.filter((v) => v.known_for_department === "Acting" && v.order <= 4)
  .map((v) => v.name).join("•");
        ...
        <div className="detail-wrap">
          <div className="detail-info">
            ...
            <ul className="detail-features">
              ...
              <li className="features-item">{allGenres}</li>
              <li className="features-item">{movieRuntime(detail.runtime)}</li>
            </ul>

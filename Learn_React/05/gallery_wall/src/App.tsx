...
import ImageWithFailback from "./components/ImageWithFailback";
...
export default function App() {
  return (
    <main className="min-h-screen bg-background">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 pb-8 sm:pb-12">
        ...
        >
          {galleryImages.map((img, index) => (
            <figure
              key={index}
              className="group bg-background rounded-lg overflow-hidden shadow-sm transition-all duration-300 relative col-span-1 row-span-1 sm:col-span-1 sm:row-span-1 md:col-span-1 md:row-span-2"
            ...
            >
              <section className="relative overflow-hidden h-full">
                ...
                />
                <section className="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></section>
                <figcaption className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent text-white transform translate-y-full group-hover:translate-y-0 transition-transform duration-300 p-3 sm:p-3 md:p-4">
                  <h3 className="mb-1">{img.caption}</h3>
                  <p className="text-white/80 text-sm leading-relaxed">{img.alt}</p>
                </figcaption>
              </section>
            </figure>
          ))}
      ...
      <footer className="text-center py-6 sm:py-8 px-4 sm:px-6 border-t border-border bg-muted/30">
        <p className="text-muted-foreground text-sm sm:text-base">
          <span className="block sm:inline">Curated Photography Collection</span>
          <span className="hidden sm:inline"> • </span>
          <span className="block sm:inline">Gallery Wall Experience</span>
          <span className="block mt-2 text-xs sm:text-sm">
            {galleryImages.length} images hidden • {galleryImages.length} displayed
          </span>
        </p>
      </footer>
    </main>
